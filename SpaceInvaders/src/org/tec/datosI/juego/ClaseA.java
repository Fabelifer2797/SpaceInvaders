...
public class ClaseA extends HileraEnemigos{
	
	private int ID = 1;
	private ListaGeneral<Graficos> listaClaseA = new ListaDoble<Graficos>();
	...
	public void CrearHilera(Graficos Nave, String Alien, String Jefe, SpaceInvaders Juego) {
		...
		listaClaseA.InsertarFinalLista(Nave, "N");
		...
		for (int f = 0; f < 1; f++) 
		{
			for (int c = 0; c < 8; c++) 
			{
				if(c == posicion) {
					...
					alien.setJefe();
					listaClaseA.InsertarFinalLista(alien,"J");
					Juego.num_aliens++;
				}
					...
					listaClaseA.InsertarFinalLista(alien,"A");
					Juego.num_aliens++;
	...
	public void AgregarMisil(Misil misil) {
		...
		listaClaseA.InsertarFinalLista(misil,"M");
